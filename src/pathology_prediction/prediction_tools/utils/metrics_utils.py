...
def metrics_report(
    ...
    if verbose:
        ...
        print(
            classification_report(targets, bin_preds, zero_division=False), flush=True
        )
...
def print_metrics(results, thresholds, pathology_names):
    ...
    metrics_dict = {}
    for t in thresholds:
        print("Metrics with `threshold = {}`".format(t), flush=True)
...
def select_best_validation_threshold(fin_targets, fin_outputs, metrics_threshold):
